...
function gamipress_get_user_points_awarded_in_loop( $user_id = 0, $points_type = '', $requirement_id = 0 ) {
	...
	if( ! isset( $GLOBALS[$multiple_awarded_key][$user_id] ) )
		$GLOBALS[$multiple_awarded_key][$user_id] = array();

    // Initialize the awarded points count for this requirement
    if( ! isset( $GLOBALS[$multiple_awarded_key][$user_id][$requirement_key] ) ) {
        $GLOBALS[$multiple_awarded_key][$user_id][$requirement_key] = 0;
    }
	
    return ( $GLOBALS[$last_points_key][$requirement_key] - $GLOBALS[$multiple_awarded_key][$user_id][$requirement_key] );
 ...
 * @return integer $user_points  The user's points on the current rules engine loop
 */
function gamipress_update_user_points_awarded_in_loop( $new_points = 0, $points_type = '', $requirement_id = 0, $user_id = 0 ) {
    ...
    if( ! isset( $GLOBALS[$multiple_awarded_key] ) )
		...
		// Initialize the last points earned array per user
	if( ! isset( $GLOBALS[$multiple_awarded_key][$user_id] ) )
		$GLOBALS[$multiple_awarded_key][$user_id] = array();
    ...
    $GLOBALS[$multiple_awarded_key][$user_id][$requirement_key] += $new_points;

    return $GLOBALS[$multiple_awarded_key][$user_id][$requirement_key];
	...
	if( ! empty( $log_type ) ) {
		...
		if ( $admin_id ) {
			...
			if( $new_points > 0 ) {
				// Points awarded
				$type = 'points_award';
				$log_meta['pattern'] = apply_filters( 'gamipress_points_awarded_log_pattern', __( '{admin} awarded {user} {points} {points_type} for a new total of {total_points} {points_type}', 'gamipress' ) );
			} else {
				// Points revoked
				$type = 'points_revoke';
				$log_meta['pattern'] = apply_filters( 'gamipress_points_revoked_log_pattern', __( '{admin} revoked {user} {points} {points_type} for a new total of {total_points} {points_type}', 'gamipress' ) );
			}
			...
			if( $new_points > 0 ) {
				// Points earned
				$type = 'points_earn';
				$log_meta['pattern'] = apply_filters( 'gamipress_points_earned_log_pattern', __( '{user} earned {points} {points_type} for a new total of {total_points} {points_type}', 'gamipress' ) );
			} else {
				// Points deducted
				$type = 'points_deduct';
				$log_meta['pattern'] = apply_filters( 'gamipress_points_deducted_log_pattern', __( '{user} deducted {points} {points_type} for a new total of {total_points} {points_type}', 'gamipress' ) );
			}
