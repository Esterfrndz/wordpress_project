...
<!-- /wp:shortcode -->',
    ...
    function resetEmailTemplate()
    {
        $data = array(
            'success' => false,
            'data' => array(
                'message' => __('Email template reset failed.', 'wp-loyalty-rules'),
            ),
        );
        if (!$this->isBasicSecurityValid('wlr_setting_nonce')) {
            $data['data']['message'] = __('Security check failed.', 'wp-loyalty-rules');
        ...
        $email_type = (string)self::$input->post('email_type', '');
        $email_type = Validation::validateInputAlpha($email_type);
        if (empty($email_type) || !$this->isValidEmailType($email_type)) {
            $data['data']['message'] = __('Email template not reset successfully.', 'wp-loyalty-rules');
            wp_send_json($data);
        }
        $status = apply_filters('wlr_reset_email_template', false, $email_type);
        if ($status) {
            $data['success'] = true;
            $data['data']['message'] = __('Email template reset successfully.', 'wp-loyalty-rules');
        }
        wp_send_json($data);
    }
