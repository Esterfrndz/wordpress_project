...
class Main extends Base
{
    ...
    function addFrontEndScripts()
    {
        ...
        $css_handlers = apply_filters('wlr_front_css_handler', [WLR_PLUGIN_SLUG . '-alertify-front', WLR_PLUGIN_SLUG . '-main-front', WLR_PLUGIN_SLUG . '-wlr-font']);
        ...
        $js_handlers = apply_filters('wlr_front_js_handler', ['wc-cart-fragments', WLR_PLUGIN_SLUG . '-main', WLR_PLUGIN_SLUG . '-alertify-front']);
            ...
            try {
                ...
                if ($user_reward_id > 0) {
                    ...
                    if (isset($reward->discount_type) && $reward->discount_type == 'points_conversion') {
                        ...
                        if ($post_point > $point) {
                            wc_add_notice(sprintf(__('%s %s used for created coupon', 'wp-loyalty-rules'), $point, $reward_helper->getPointLabel($point)));
                        }
                    }
                    $reward_helper->createCartUserReward($user_reward, $user_email);
    ...
    function getPointConversionDiscountAmount($discount, $price_to_discount, $item, $is_false, $coupon)
    {
        ...
        if (empty($user_reward)) return $discount;
    ...
    function getDiscountForItem($max_item_discount, $max_cart_discount, $item, $coupon)
    {
        ...
        $code = self::$woocommerce->isMethodExists($coupon, 'get_code') ? $coupon->get_code() : '';
        if (empty($code)) return 0;
        // $cart_item_qty = self::$woocommerce->isMethodExists(WC()->cart, 'get_cart_contents_count') ? WC()->cart->get_cart_contents_count() : 1;
        $cart_item_discounts = $this->getItemDiscountAmount($item['key'], $max_cart_discount);
    ...
    function validateRewardCoupon($is_valid, $coupon, $discount)
    {
        ...
        if (empty($free_product_list)) return $is_valid;
        foreach ($free_product_list as $product_id => $free_product) {
            $product = self::$woocommerce->getProduct($product_id);
            if (self::$woocommerce->isMethodExists($product, 'is_in_stock') && !$product->is_in_stock()) {
                return false;
            }
        }
        return $is_valid;
    ...
    function applyCartCoupon()
    {
        ...
        if (!empty($discount_code) && self::$woocommerce->isValidCoupon($discount_code) && !empty($cart) && $cart->get_cart() && !self::$woocommerce->hasDiscount($discount_code)) {
            ...
            \Wlr\App\Helpers\Rewards::setCouponRemainingAmount($discount_code, 0);
