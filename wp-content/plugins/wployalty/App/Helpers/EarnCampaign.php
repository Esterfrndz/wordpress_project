...
class EarnCampaign extends Base
{
    ...
    function addEarnCampaignReward($action_type, $reward, $campaign_id, $action_data, $force_generate_coupon = false)
    {
        ...
        if (!is_array($action_data) || !isset($reward->id) || $reward->id <= 0 || empty($action_data['user_email'])
            || empty($action_type) || !$this->is_valid_action($action_type)) {
            return false;
        }
    ...
    function checkFreeProductCoupon($status, $reward)
    {
        ...
        $free_products = self::$woocommerce_helper->isJson($reward_data->free_product) ?
